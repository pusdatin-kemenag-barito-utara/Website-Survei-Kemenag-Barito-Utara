...
export function NavLanguageToggle() {
  ...
  const handleSelectLanguage = (newLocale: 'id' | 'en') => {
    setLocale(newLocale)
    Analytics.switchLanguage(newLocale)
  }
    ...
    <div className="hidden lg:block">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-40 rounded-2xl p-1.5 shadow-xl border-slate-200">
          <DropdownMenuItem onClick={() => handleSelectLanguage('id')} className={cn("gap-2.5 cursor-pointer rounded-xl py-2 text-xs font-bold", locale === 'id' && "bg-emerald-50 text-emerald-700")}>
            <Image src="https://flagcdn.com/w20/id.png" alt="ID" width={18} height={13} className="w-4 rounded-xs shadow-xs" unoptimized />
            <span>Indonesia</span>
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => handleSelectLanguage('en')} className={cn("gap-2.5 cursor-pointer rounded-lg py-2 text-xs font-bold", locale === 'en' && "bg-emerald-50 text-emerald-700")}>
