...
export default function AdminPertanyaanPage() {
  ...
  const initialSortedUnsur = cachedUnsur
  ...
  const initialSelectedUnsur = selectedUnsurId || (initialSortedUnsur.length > 0 ? initialSortedUnsur[0].id : '')
  ...
  const [questions, setQuestions] = useState<Question[]>(() => {
    if (cachedQuestions && initialSelectedUnsur) {
      return cachedQuestions.filter(q => q.unsur_id === initialSelectedUnsur)
    }
    return []
  })
  const [loading, setLoading] = useState(() => !cachedQuestions)
  const [filterUnsur, setFilterUnsur] = useState(initialSelectedUnsur)
  ...
  const fetchQuestions = useCallback(async (targetId?: string, force = false) => {
    const idToUse = targetId || filterUnsur
    if (!idToUse) { setQuestions([]); setLoading(false); return }
    try {
      const data = await fetchCachedAdminQuestions(force)
  ...
  useEffect(() => {
    ...
    fetchQuestions(filterUnsur)
  }, [filterUnsur, fetchQuestions])
