...
function getDates(y: string, p: string) {
...
export default function ArsipPage() {
  ...
  switch (period.toLowerCase()) {
    case "q1":
      periodName = isEn ? "Quarter 1 (Q1)" : "Triwulan I";
      break;
    case "q2":
      periodName = isEn ? "Quarter 2 (Q2)" : "Triwulan II";
      break;
    case "q3":
      periodName = isEn ? "Quarter 3 (Q3)" : "Triwulan III";
      break;
    case "q4":
      periodName = isEn ? "Quarter 4 (Q4)" : "Triwulan IV";
      break;
    case "sem1":
    case "s1":
  ...
  const currentDates = getDates(yearStr, period);
  const initArchive = getCachedArchiveResultsSync(currentDates.rawStart, currentDates.rawEnd);
  ...
  const [summary, setSummary] = useState<IndexSummary[]>(
    initArchive
      ? [
          {
            index_type: "IPKP",
            nilai_index: initArchive.ipkp_score / 25,
            nilai_konversi: initArchive.ipkp_score,
            mutu:
              initArchive.ipkp_score >= 88.31
                ? "A"
                : initArchive.ipkp_score >= 76.61
                  ? "B"
                  : initArchive.ipkp_score >= 65.0
                    ? "C"
                    : "D",
            mutu_pelayanan:
              initArchive.ipkp_score >= 88.31
                ...
                : initArchive.ipkp_score >= 76.61
                  ...
                  : initArchive.ipkp_score >= 65.0
                    ? "Kurang Baik"
                    : "Tidak Baik",
            total_responden: initArchive.total_responses,
          },
          {
            index_type: "IPAK",
            nilai_index: initArchive.ipak_score / 25,
            nilai_konversi: initArchive.ipak_score,
            mutu:
              initArchive.ipak_score >= 88.31
                ? "A"
                : initArchive.ipak_score >= 76.61
                  ? "B"
                  : initArchive.ipak_score >= 65.0
                    ? "C"
                    : "D",
            mutu_pelayanan:
              initArchive.ipak_score >= 88.31
  ...
  const [byService, setByService] = useState<IndexByService[]>(initArchive?.by_service || []);
  const [trend, setTrend] = useState<IndexTrend[]>(initArchive?.trend || []);
  const [unsurSummary, setUnsurSummary] = useState<UnsurSummary[]>(initArchive?.unsur_summary || []);
  const [demoSummary, setDemoSummary] = useState<DemographicSummary[]>(initArchive?.demographics || []);
  const [allServices, setAllServices] = useState<{ id: string; name: string }[]>(initServices || []);
  const [totalResponses, setTotalResponses] = useState(initArchive?.total_responses || 0);
  const [loading, setLoading] = useState(!initArchive);
