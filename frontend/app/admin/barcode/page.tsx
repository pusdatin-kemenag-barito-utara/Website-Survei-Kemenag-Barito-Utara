...
export default function AdminBarcodePage() {
  ...
  useEffect(() => {
    async function fetchServices() {
      try {
        const res = await apiFetch<Service[] | { services: Service[] }>(
          "/survey/services",
        );
        const list = Array.isArray(res)
          ? res
          : res && "services" in res && Array.isArray(res.services)
            ? res.services
            : [];
        setServices(list);
      } catch (err) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm font-medium mt-0.5">
              Kelola dan unduh media QR Code resmi berlogo untuk banner/poster
              pelayanan publik.
            </p>
          </div>
        </div>
                    ...
                    <SelectValue placeholder="Pilih Layanan Target">
                      {selectedServiceId === "all"
                        ...
                        : services.find((s) => s.id === selectedServiceId)
                            ?.name || "Pilih Layanan Target"}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent className="rounded-2xl p-1.5 shadow-xl max-h-72">
                    <SelectItem
                      value="all"
                      className="rounded-xl py-2 font-bold text-xs sm:text-sm"
                    >
                      🌐 Semua Layanan (Kuesioner Umum)
                    </SelectItem>
                    {services.map((s) => (
                      <SelectItem
                        key={s.id}
                        value={s.id}
                        className="rounded-xl py-2 text-xs sm:text-sm font-semibold"
                      >
                        {s.name}
                      </SelectItem>
                    ))}
                  ...
                  {[
                    ...
                    >
                      {item.img ? (
                        ...
                        <div className="size-8 flex items-center justify-center font-bold text-xs text-slate-400">
                          ∅
                        </div>
                      )}
            ...
            <CardContent className="p-6 sm:p-10 flex flex-col items-center justify-center space-y-6">
              ...
              <div className="space-y-1 max-w-sm text-center">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                  Bantu kami meningkatkan mutu pelayanan dengan memberikan
                  ulasan &amp; masukan objektif.
                </p>
              </div>
